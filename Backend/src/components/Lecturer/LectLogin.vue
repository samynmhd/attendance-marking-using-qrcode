          ...
          <div class="col-md-12">
            ...
            <form role="form" autocomplete="off" @submit="login">
              <div class="form-group">
                <label for="id" class="sr-only">Lecturer ID</label>
                <input type="text" v-model="id" id="email" class="form-control" placeholder="Lecturer id">
              </div>
              <div class="form-group">
...
export default {
  ...
  methods: {
    login(evt) {
      ...
      const baseURL = location.host || `localhost:3000`;
      axios.post(`https://${baseURL}/api/auth/lecturer/login`, data)
        .then((res) => {
          var self = this;
          console.log(res);
