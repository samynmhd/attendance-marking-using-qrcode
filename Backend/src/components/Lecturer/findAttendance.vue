...
export default {
  ...
  created() {
    if (!this.$session.has('jwt')) {
      ...
      let test = this.test;
      axios.get(`https://${location.host}/api/lect/subjects/${this.user.school}`)
        .then(resp => {
    ...
    find(evt) {
      ...
      if (this.subject == '' && this.date == '') {
        ...
        if (this.subject != '' && this.date == '') {
          ...
          axios.get(`https://${location.host}/api/lect/findAttendance/${sub}`)
            .then(res => {
              this.data = res.data;
              this.bool = false;
            })
          ...
          this.errors = '';
          axios.get(`https://${location.host}/api/lect/findAttendance/${this.subject}/${this.date}`)
            .then(res => {
              this.data = res.data;
              this.bool = false;
            })
